...
rho2 = 1024 # salt
h1s = np.asarray([0.15, 0.15]) #fresh water height
...
for interface in interfaces:
    ...
    for velocity in velocities:
        ...
        for mesh in meshes:
            filename  = '/home/karl/OpenFOAM/karl-5.0/run/MT/MT_PK/twoPhaseIF2/freshSaltMixingMeshSensitivityZG/%s/interface%s/postProcessingVelocity%s/forceCoeffs/0/forceCoeffs.dat' %(mesh, interface, velocity)
    ...
    for interface in interfaces:
        ...
        for mesh in meshes:
            ax.plot(resultsDict[interface][velocity][mesh]['time'], \
                 resultsDict[interface][velocity][mesh]['cd'], \
        ...
        ax.set_title('Pycnocline %sm \n Fr %.2f' %(titles[counterTitles], FrS),\
                     fontsize=fontSize)
        #fig.tight_layout()
        fig.savefig('/home/karl/OpenFOAM/karl-5.0/run/MT/MT_PK/plots/meshSensitivity/U%sInterface%sZG' %(velocity, interface),\
                    bbox_inches = 'tight')
        counterTitles += 1
    velocityCounter += 1
